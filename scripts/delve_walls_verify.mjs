...
const browser = await puppeteer.launch({
  ...
  args: [
    '--window-size=1280,820', '--use-angle=swiftshader', '--enable-unsafe-swiftshader',
...
const entered = await page.evaluate(() => {
  ...
  return run ? { ok: true, modules: run.modules.length, x: Math.round(sim.player.pos.x) } : { ok: false };
});
check('entered delve run', entered.ok, JSON.stringify(entered));

// Let the renderer prebuild the module interiors.
await page.waitForFunction(
  () => (window.__game.renderer?.builtInteriors?.size ?? 0) > 0,
  { timeout: 20000, polling: 400 },
).catch(() => {});
await sleep(1500);

function moduleBounds(page) {
  return page.evaluate(() => {
    ...
    const layout = sim.constructor && L ? L : null;
    return { run: { moduleIndex: run.moduleIndex, modules: run.modules }, originX: run.origin.x, originZ: run.origin.z };
  });
}

// Roam: hold each direction so the body is driven into every wall, sampling pos.
async function roamAndSample(label) {
  ...
  for (const k of keys) {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const s = await page.evaluate(() => {
        ...
        const run = sim.delveRunForPlayer(sim.playerId);
        const id = run.modules[run.moduleIndex];
        const layout = sim.delveLayoutFor ? null : null;
...
async function activeBounds() {
  return page.evaluate(() => {
    ...
    return {
      ox: run.origin.x, oz: run.origin.z + zBase,
      halfX: wallX - 1, zMin: L.zMin, zMax: L.zMax, mi: run.moduleIndex,
    };
  });
}
...
async function attemptMove(fromX, fromZ, toX, toZ) {
  return page.evaluate((fx, fz, tx, tz, r) => {
    const sim = window.__game.sim;
    const p = sim.player;
    return sim.resolveMove(fx, fz, tx, tz, r, p);
  }, fromX, fromZ, toX, toZ, R);
}
...
check('room0: side wall stops eastward charge', res.x <= b.ox + b.halfX - R + 0.05 && res.x > b.ox + b.halfX - R - 1,
  `x=${(res.x - b.ox).toFixed(2)} limit=${(b.halfX - R).toFixed(2)}`);
// West.
res = await attemptMove(b.ox, midZ, b.ox - 200, midZ);
check('room0: side wall stops westward charge', res.x >= b.ox - (b.halfX - R) - 0.05 && res.x < b.ox - (b.halfX - R) + 1,
...
check('room0: front wall seals the south exit (no gap escape)',
  (res.z - b.oz) >= b.zMin + 1 - 0.05, `z=${(res.z - b.oz).toFixed(2)} seal=${(b.zMin + 1)}`);
...
check('gap: cannot stand/strafe out through the missing side walls',
...
res = await attemptMove(b.ox, b.oz + b.zMin + 8, b.ox, b.oz + b.zMin - 300, b.ox);
check('module1: cannot backtrack south into the previous room',
  (res.z - b.oz) >= b.zMin + 1 - 0.05, `z=${(res.z - b.oz).toFixed(2)} seal=${(b.zMin + 1)}`);
