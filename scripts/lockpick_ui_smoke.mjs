...
const anteDom = await page.evaluate(() => {
  ...
  return { visible: el.style.display === 'block', anteBtns: el.querySelectorAll('[data-ante]').length, text: el.textContent.slice(0, 80) };
});
...
await page.evaluate(() => {
  const visible = [
    { col: 0, row: 1, kind: 'open' }, { col: 0, row: 2, kind: 'open' }, { col: 0, row: 3, kind: 'open' },
    { col: 1, row: 1, kind: 'open' }, { col: 1, row: 2, kind: 'open' }, { col: 1, row: 3, kind: 'open' },
    { col: 2, row: 2, kind: 'gate' },
    { col: 3, row: 1, kind: 'open' }, { col: 3, row: 2, kind: 'open' }, { col: 3, row: 3, kind: 'open' },
    { col: 4, row: 2, kind: 'open' }, { col: 4, row: 3, kind: 'open' }, { col: 4, row: 4, kind: 'open' },
  ];
  window.__game.hud.handleEvents([{
    type: 'lockpickSession', sessionId: 't1', objectId: 1, w: 11, h: 6,
    col: 0, row: 2, lives: 1, lootTier: 'premium',
    allowed: ['hardSet', 'set', 'steady', 'ease', 'drop'], visible,
  }]);
});
await sleep(300);
...
await page.evaluate(() => {
  const visible = [
    { col: 0, row: 2, kind: 'open' }, { col: 1, row: 2, kind: 'open' }, { col: 2, row: 2, kind: 'gate' },
    { col: 3, row: 2, kind: 'open' }, { col: 4, row: 3, kind: 'open' }, { col: 5, row: 3, kind: 'open' },
  ];
  window.__game.hud.handleEvents([{ type: 'lockpickStep', sessionId: 't1', col: 1, row: 2, lives: 1, result: 'advanced', visible }]);
});
await sleep(200);
