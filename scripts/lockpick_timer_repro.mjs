...
const browser = await puppeteer.launch({
  ...
  args: ['--window-size=1280,800', '--use-angle=swiftshader', '--enable-unsafe-swiftshader', '--no-first-run'],
...
try {
  ...
  const dump = await page.evaluate(() => ({
    ...
    visiblePanels: [...document.querySelectorAll('[id]')].filter((e) => e.offsetParent).map((e) => e.id).slice(0, 20),
  }));
...
const spawnChest = async () => page.evaluate(() => {
...
console.log(tA.stillInProgress
...
const nextCorrectKey = async () => page.evaluate((DELTA) => {
  const sim = window.__game.sim;
  const run = sim.delveRunForPlayer(sim.player.id);
  const s = run?.lockpick; if (!s) return null;
  const spec = s.pages[s.pageIndex];
  const W = spec.open.length;
  const deltas = spec.tier.allowedActions.map((a) => DELTA[a]);
  let reach = new Set([spec.startRow]); const parents = [new Map()];
  for (let c = 1; c < W; c++) {
...
for (let i = 0; i < 40; i++) {
  ...
  if (i < 3) console.log(`  move ${pressed}: timer ${justAfter}s just after press -> ${beforeNext}s after ~4s wait`);
  const ended = await page.evaluate((id) => {
    const sim = window.__game.sim;
    const run = sim.delveRunForPlayer(sim.player.id);
    return { gone: !sim.lockpickState, looted: !!run.objectState[id]?.looted };
  }, chestB);
  if (ended.gone && !ended.looted) { failedMid = true; break; }
...
console.log(`B: pressed ${pressed} CORRECT keys with 2s human pauses -> looted=${tB.looted} failedMidway=${failedMid}`);
console.log(tB.looted
