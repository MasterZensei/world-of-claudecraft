...
import { solveLockActions, ANTE_TO_STEP_TIMEOUT_MS } from '../src/sim/lockpick';
...
function budgetTicks(sim: Sim): number {
  return Math.ceil(sim.lockpickState!.stepTimeoutMs! / (DT * 1000));
}
...
describe('lockpick per-step clock is enforced by the sim tick (server-authoritative)', () => {
  ...
  it('exposes the authoritative per-step budget on the view (HUD renders, never decides)', () => {
    ...
    expect(sim.lockpickState?.stepTimeoutMs).toBe(ANTE_TO_STEP_TIMEOUT_MS[run.lockpick.ante as 1 | 2 | 3]);
