...
import { stepLock, type LockSpec, type LockTierSpec } from '../src/sim/lockpick';
...
describe('generateLock, traps + multi-page', () => {
  it('places exactly trapCount traps, none on the solution path, lock still solvable', async () => {
    ...
    for (const seed of [1, 2, 3, 99, 12345, 654321]) {
      ...
      let col = 0, row = sp.startRow, result = 'advanced';
      for (const a of solveLockActions(sp)!) { const st = stepLock(sp, col, row, a); col = st.col; row = st.row; result = st.result; expect(['advanced', 'success']).toContain(result); }
  ...
  it('generateLockPages produces N distinct deterministic boards', async () => {
    ...
    const t: LockTierSpec = { cols: 12, rows: 6, width: 1, gateCount: 2, visibilityWindow: 4, trapCount: 3, allowedActions: ['hardSet', 'set', 'steady', 'ease', 'drop'] };
