...
function actionForCol(run: { lockpick: { pages: unknown[]; pageIndex: number } | null }, col: number): PickAction {
  const spec = run.lockpick!.pages[run.lockpick!.pageIndex] as Parameters<typeof solveLockActions>[0];
...
class LockpickHudBridge {
  ...
  pickForLiveCol(run: { lockpick: { col: number; pages: unknown[]; pageIndex: number } | null }): PickAction {
