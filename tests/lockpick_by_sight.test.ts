...
import { generateLock, stepLock, visibleCells, ACTION_DELTA, type LockTierSpec } from '../src/sim/lockpick';
...
function winsBySight(spec: ReturnType<typeof generateLock>): boolean {
  ...
  while (col < W - 1 && guard++ < W * 3) {
    ...
    let best = -1;
    for (const [d, n] of optionsByFirstDelta) if (n > best) { best = n; bestDelta = d; }
  ...
  for (const [name, tier] of Object.entries(LOCKPICK_TIER_PRESETS) as [string, LockTierSpec][]) {
    it(`${name} preset: a fog-limited player avoiding visible traps clears every seed`, () => {
      ...
      for (const seed of SEEDS) {
        ...
        if (winsBySight(spec)) won++; else failed.push(seed);
      }
      expect(failed, `${failed.length} seeds unwinnable by sight: ${failed.slice(0, 5).join(', ')}`).toEqual([]);
