...
const base: LockpickView = {
  sessionId: 'lp_1_0', objectId: 1, w: 16, h: 6, col: 0, row: 3,
  page: 1, pageCount: 3, tries: 1, triesTotal: 1, lootTier: 'premium',
  allowed: ['set'], visible: [], stepTimeoutMs: 15000,
};
...
describe('lockpickTimerKey (per-move clock refill)', () => {
  ...
  it('is stable while nothing about the timed move changed (no needless restart)', () => {
    ...
    expect(lockpickTimerKey({ ...base, row: 5, visible: [{ col: 0, row: 1, kind: 'open' }] }))
      .toBe(lockpickTimerKey(base));
  });
});
...
describe('lockpickRenderSig', () => {
  ...
  it('changes when any painted field moves', () => {
    ...
    expect(lockpickRenderSig({ ...base, visible: [{ col: 0, row: 1, kind: 'open' }] })).not.toBe(sig);
  });
});
