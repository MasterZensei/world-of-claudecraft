import {
  ...
  DELVE_LIST, DELVE_MODULES, DELVE_MODULE_GAP, DELVE_MODULE_Z_START, DELVES, DELVE_SHOPS, DELVE_SLOT_COUNT,
  DUNGEONS, DUNGEON_LIST, DungeonDef, arenaOrigin, delveAt, delveOrigin, delveModuleZOffset, dungeonAt,
  resolveDelveShopOffers, delveShopGateUnlocked,
...
import type { AccountCosmetics, DelveCompanionInfo, DelveRunInfo, LeaderboardEntry, LockpickView } from '../world_api';
import {
...
export class Sim {
  ...
  private resolveMove(
    fromX: number, fromZ: number, nx: number, nz: number, r: number, e: Entity, ignoreFences = false,
  ): { x: number; z: number } {
  ...
  enterDelve(delveId: string, tierId: string, pid?: number): void {
    ...
    if (gate) { this.error(r.meta.entityId, gate); return; }
    if (!delve.tiers.some((t) => t.id === tierId)) { this.error(r.meta.entityId, 'Unknown delve tier.'); return; }
    ...
    if (tierDef?.minPlayerLevel && r.e.level < tierDef.minPlayerLevel) {
      this.error(r.meta.entityId, `You must be level ${tierDef.minPlayerLevel} to enter ${delve.name} on ${tierDef.label}.`);
      return;
    }
    const key = this.instanceKeyFor(r.meta.entityId);
    ...
    if (!run) {
      ...
      if (!run) { this.error(r.meta.entityId, `All instances of ${delve.name} are busy. Try again soon.`); return; }
  ...
  releaseSpiritInDelve(pid: number): void {
    ...
    p.resource = p.resourceType === 'mana'
      ? Math.round(p.maxResource * 0.5)
      : p.resourceType === 'energy' ? 100 : 0;
    p.targetId = null;
    p.combatTimer = 99;
    p.inCombat = false;
  ...
  companionUpgrade(companionId: string, pid?: number): void {
    ...
    if (rank >= DELVE_COMPANION_MAX_RANK) { this.error(r.meta.entityId, 'This companion is already fully upgraded.'); return; }
    const next = rank + 1;
  ...
  delveBuyShopItem(delveId: string, itemId: string, pid?: number): void {
    ...
    if (!entry) { this.error(meta.entityId, 'That item is not sold here.'); return; }
    const def = ITEMS[itemId];
    if (!def) { this.error(meta.entityId, 'That item is not for sale.'); return; }
