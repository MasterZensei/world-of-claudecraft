...
/** Clamp a third-person chase camera inside a module footprint (delve/dungeon). */
export function clampChaseCameraInModule(
  cx: number,
  cz: number,
  px: number,
  pz: number,
  ox: number,
  oz: number,
  layout: Pick<DungeonLayout, 'zMin' | 'zMax' | 'wallX'>,
  clearance = 2,
  camOrbit = 12,
  camPitch = 0,
): { cx: number; cz: number } {
  const walkX = layout.wallX != null ? layout.wallX - DUNGEON_WALL_HW : DUNGEON_WALK_HALF_X;
  const halfW = walkX - clearance;
  const localPx = px - ox;
  const localPz = pz - oz;
  const horizOrbit = camOrbit * Math.cos(camPitch);
  const nearBand = horizOrbit * 0.45;
  let outCx = Math.min(Math.max(cx, ox - halfW), ox + halfW);
  let outCz = Math.min(Math.max(cz, oz + layout.zMin + clearance), oz + layout.zMax - clearance);
  // When the player hugs a wall, park the lens on the center-aisle side with a
  // full horizontal chase offset. AABB-only clamp (or a 1.5u nudge) still lets
  // the 12yd orbit frustum look past the wall into black fog — module 0 right
  // aisle — and stacks nameplates when the lens is nearly collinear with mobs.
  if (localPx > halfW - nearBand) {
    const inset = Math.min(horizOrbit, localPx - clearance);
    outCx = Math.min(outCx, px - inset);
  }
  if (localPx < -halfW + nearBand) {
    const inset = Math.min(horizOrbit, -localPx - clearance);
    outCx = Math.max(outCx, px + inset);
  }
  if (localPz > layout.zMax - clearance - nearBand) {
    const inset = Math.min(horizOrbit, layout.zMax - clearance - localPz);
    outCz = Math.min(outCz, pz - inset);
  }
  if (localPz < layout.zMin + clearance + nearBand) {
    const inset = Math.min(horizOrbit, localPz - layout.zMin - clearance);
    outCz = Math.max(outCz, pz + inset);
  }
  return { cx: outCx, cz: outCz };
}
