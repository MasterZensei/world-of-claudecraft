...
const LOOT_ARCHETYPE: Record<PlayerClass, 'WAR' | 'ROG' | 'MAG'> = {
  warrior: 'WAR', paladin: 'WAR', shaman: 'WAR',
  rogue: 'ROG', hunter: 'ROG',
  mage: 'MAG', priest: 'MAG', warlock: 'MAG', druid: 'MAG',
};
...
export function delveChestItemsForTier(
  ...
  if (bountiful) {
    if (arch === 'WAR') return [{ itemId: 'deacon_reliquary_helm', count: 1 }, { itemId: 'reliquary_plate_chest', count: 1 }];
    if (arch === 'MAG') return [{ itemId: 'varric_shadow_cowl', count: 1 }, { itemId: 'reliquary_cloth_chest', count: 1 }];
    return [{ itemId: 'reliquary_leather_chest', count: 1 }, { itemId: 'reliquary_gloves_rog', count: 1 }];
  }
  if (tier === 'premium') {
    if (arch === 'WAR') return [{ itemId: rng.chance(0.5) ? 'deacon_reliquary_helm' : 'reliquary_plate_chest', count: 1 }];
    if (arch === 'ROG') return [{ itemId: 'reliquary_leather_chest', count: 1 }, { itemId: 'reliquary_gloves_rog', count: 1 }];
    return [{ itemId: rng.chance(0.5) ? 'varric_shadow_cowl' : 'reliquary_cloth_chest', count: 1 }];
  }
  if (tier === 'medium') {
    const itemId = arch === 'WAR' ? 'reliquary_plate_chest'
      : arch === 'ROG' ? 'reliquary_leather_chest'
