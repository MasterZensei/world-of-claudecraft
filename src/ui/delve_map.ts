...
export function delveSchematicStatic(
  ...
  for (const t of layout.tombs) {
    ...
    prims.push({ kind: 'rect', x: cx - tw / 2, y: cy - th / 2, w: tw, h: th, fill: '#2e2820', stroke: '#4a4030', strokeWidth: 0.5 });
  }
  ...
  const dr = Math.max(4, Math.min(canvasSize * 0.12, (dais.r / (layout.zMax - layout.zMin)) * (canvasSize - pad * 2)));
  ...
  prims.push({ kind: 'circle', cx: exCx, cy: exCy, r: Math.max(3, canvasSize * 0.025), fill: '#7a50c8', stroke: '#b090e8', strokeWidth: 1 });
  prims.push({ kind: 'text', cx: exCx, cy: exCy - Math.max(5, canvasSize * 0.05), text: 'N', fill: '#b090e8', font: `bold ${Math.max(8, Math.round(canvasSize * 0.08))}px Georgia` });
