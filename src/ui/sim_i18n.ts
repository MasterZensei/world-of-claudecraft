...
export const QUEST_EXTRA: Record<SupportedLanguage, Record<QuestExtraKey, string>> = {
  ...
  ko_KR: {
    ...
    malricYell: '{name}이(가) 외칩니다. "죽음은 결코 내 왕을 데려가지 못한다! 의식은 계속되어야 한다!"',
...
type RaidExtraKey =
  | 'converted' | 'memberMoved' | 'needFullParty' | 'leaderConvert'
  | 'alreadyRaid' | 'notInRaid' | 'leaderAdjust' | 'groupFull' | 'yourRaidFull' | 'thatRaidFull'
  | 'noDuelArena' | 'noStandardDungeons' | 'mustConvert'
  | 'royalDoorSealed' | 'locked' | 'engagedSealed' | 'mustFall';
...
const RAID_EXTRA: Record<SupportedLanguage, Record<RaidExtraKey, string>> = {
  ...
  de_DE: {
    ...
    needFullParty: 'Ihr braucht eine volle Fuenfergruppe, bevor Ihr sie in einen Schlachtzug umwandeln koennt.',
...
const RULES: Rule[] = [
  ...
  { re: /^(.+) has been moved to raid group (.+)\.$/, build: (m) => tRaidExtra('memberMoved', { name: m[1], group: m[2] }) },
  { re: /^You need a full party of five before converting to raid\.$/, build: () => tRaidExtra('needFullParty') },
  ...
  { re: /^Raid groups cannot enter standard dungeons\.$/, build: () => tRaidExtra('noStandardDungeons') },
  { re: /^You must convert your party to a raid group first\.$/, build: () => tRaidExtra('mustConvert') },
  ...
  { re: /^Nythraxis is engaged — the royal door has sealed shut\.$/, build: () => tRaidExtra('engagedSealed') },
  { re: /^The royal door is sealed — Nythraxis must fall first\.$/, build: () => tRaidExtra('mustFall') },
