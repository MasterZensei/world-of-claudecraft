...
export class LockpickWindow {
  ...
  private syncTimer(): void {
    const view = this.deps.getState();
    if (!view) { this.lastTimerKey = ''; this.stopTimer(); return; }
  ...
  private renderBoard(feedback = '', tone: 'good' | 'bad' | 'win' = 'good'): void {
    ...
    if (!el) return;
    const view = this.deps.getState();
    if (!view) { this.deps.onClose(); return; }
    ...
    const timerBlock = timerSecs != null
      ...
      : '';
    el.innerHTML = `<div class="panel-title"><span>${esc(t('lockpickUi.boardTitle', { tier: this.deps.tierName(view.lootTier) }))}</span>`
  ...
  private startTimer(): void {
    const view = this.deps.getState();
    if (!view || view.stepTimeoutMs == null) { this.stopTimer(); return; }
