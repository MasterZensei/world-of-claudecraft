...
export function entityTranslationManifest(): EntityTranslationManifestEntry[] {
  ...
  for (const delve of Object.values(DELVES).sort(compareById)) {
    entries.push(entry('delve', delve.id, 'name', delve.name, 'phase9', entityTranslationKey({ kind: 'delve', id: delve.id, field: 'name' })));
    entries.push(entry('delve', delve.id, 'enterText', delve.enterText, 'phase9', entityTranslationKey({ kind: 'delve', id: delve.id, field: 'enterText' })));
    entries.push(entry('delve', delve.id, 'leaveText', delve.leaveText, 'phase9', entityTranslationKey({ kind: 'delve', id: delve.id, field: 'leaveText' })));
  }
  return entries;
}
