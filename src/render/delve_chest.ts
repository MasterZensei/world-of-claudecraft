...
/** §7.6 Bountiful Coffer — ultra-rare = epic PURPLE, never the gold/yellow chest.
 * Stone reads dark amethyst; the trim/lock glow with the classic epic purple. */
const CHEST_COFFER_STONE = 0x46355f;
const CHEST_COFFER_TRIM = 0xa335ee;
const CHEST_COFFER_GLOW = 0x9a4ff0;
/** Multiplier laid over a baked albedo map so the violet reads through without
 * flattening the texture detail. */
const CHEST_COFFER_MAP_TINT = 0xb487e6;
...
function convertMaterial(src: THREE.Material, locked: boolean, bountiful: boolean): THREE.Material {
  const s = src as THREE.MeshStandardMaterial;
  const gold = isGoldTrim(s);
  const hasMap = !!s.map;
  const tint = bountiful
    ? (gold ? CHEST_COFFER_TRIM : CHEST_COFFER_STONE)
    : (gold
  ...
  return surfaceMat({
    ...
    // A Coffer multiplies the map by violet so the epic hue reads through.
    color: hasMap ? (bountiful ? CHEST_COFFER_MAP_TINT : 0xffffff) : tint,
    map: s.map ?? undefined,
    normalMap: s.normalMap ?? undefined,
    roughness: gold ? 0.75 : (locked ? 0.9 : 0.84),
    metalness: 0,
    emissive: bountiful ? CHEST_COFFER_GLOW : (gold ? (locked ? 0x1a1000 : 0x332200) : 0x000000),
    emissiveIntensity: bountiful ? (gold ? 0.32 : 0.14) : (gold ? (locked ? 0.1 : 0.12) : 0),
...
function buildProceduralChestMesh(locked: boolean, bountiful: boolean): THREE.Group {
  const group = new THREE.Group();
  const wood = bountiful ? CHEST_COFFER_STONE : (locked ? 0x8a8580 : 0x8a7050);
  const trim = bountiful ? CHEST_COFFER_TRIM : (locked ? 0x7a7568 : 0xc9a040);
  const lidWood = bountiful ? 0x5a467e : (locked ? 0x989088 : 0x9a8060);
  ...
  for (const y of [0.14, 0.42]) {
    const band = new THREE.Mesh(
      ...
      surfaceMat({
        color: trim,
        roughness: 0.75,
        emissive: bountiful ? CHEST_COFFER_GLOW : 0x000000,
        emissiveIntensity: bountiful ? 0.28 : 0,
        flatShading: !GFX.standardMaterials,
      }),
    );
    band.position.y = y;
  ...
  const lock = new THREE.Mesh(
    ...
    surfaceMat({
      color: bountiful ? CHEST_COFFER_TRIM : (locked ? 0x8a8078 : 0xffd060),
      roughness: 0.55,
      emissive: bountiful ? CHEST_COFFER_GLOW : (locked ? 0 : 0x332200),
      emissiveIntensity: bountiful ? 0.55 : (locked ? 0 : 0.25),
      flatShading: !GFX.standardMaterials,
    }),
  );
...
function prepareChest(locked: boolean, bountiful: boolean): THREE.Group | null {
  if (!ENABLE_MESHY_CHEST_GLB) return null;
  const key = `${locked ? 'L' : 'O'}${bountiful ? 'B' : ''}`;
  ...
  root.traverse((o) => {
    ...
    if (Array.isArray(src)) {
      mesh.material = src.map((m) => convertMaterial(m, locked, bountiful));
    } else {
      mesh.material = convertMaterial(src as THREE.Material, locked, bountiful);
    }
...
/** Build the finale reward chest. `bountiful` paints it as the §7.6 purple Coffer. */
export function buildDelveChestMesh(locked: boolean, bountiful = false): { group: THREE.Group; height: number } {
  const group = new THREE.Group();
  const template = prepareChest(locked, bountiful);
  if (template) {
    group.add(template.clone(true));
  } else {
    const chest = buildProceduralChestMesh(locked, bountiful);
