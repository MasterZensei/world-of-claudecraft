...
function stoneMat(color: number): THREE.Material {
  return surfaceMat({ color, roughness: 0.92, metalness: 0.0, flatShading: !GFX.standardMaterials });
}
...
function buildLockedDoor(): { group: THREE.Group; height: number } {
  ...
  const crossbar = new THREE.Mesh(
    new THREE.BoxGeometry(totalW, 0.32, 0.38),
    rustyIron,
  );
  ...
  for (const sx of [-totalW / 2 + 0.25, totalW / 2 - 0.25]) {
    const post = new THREE.Mesh(
      new THREE.BoxGeometry(0.5, totalH, 0.5),
      darkIron,
    );
  ...
  for (let i = 0; i < barCount; i++) {
    ...
    const bar = new THREE.Mesh(
      new THREE.BoxGeometry(barW, barH, barW),
      rustyIron,
    );
    ...
    const tip = new THREE.Mesh(
      new THREE.ConeGeometry(barW * 0.55, 0.6, 4),
      darkIron,
    );
  ...
  for (const sx of [-totalW / 2 + 1.2, totalW / 2 - 1.2]) {
    for (let ci = 0; ci < 4; ci++) {
      const link = new THREE.Mesh(
        new THREE.TorusGeometry(0.22, 0.07, 4, 6),
        darkIron,
      );
...
function buildCrackedGrave(entityId: number): { group: THREE.Group; height: number } {
  ...
  const mound = new THREE.Mesh(
    new THREE.CylinderGeometry(1.1, 1.6, 0.28, 12),
    dirtMat,
  );
  ...
  const crack = new THREE.Mesh(
    new THREE.BoxGeometry(0.06, 0.9, 0.24),
    stoneCracked,
  );
...
function buildModuleExit(): { group: THREE.Group; height: number } {
  ...
  for (const sx of [-1.8, 1.8]) {
    const pillar = new THREE.Mesh(
      new THREE.BoxGeometry(0.6, 5.0, 0.55),
      archStone,
    );
  ...
  // Top lintel.
  const lintel = new THREE.Mesh(
    new THREE.BoxGeometry(4.2, 0.55, 0.55),
    archStone,
  );
  ...
  const rubbleLayout: [number, number, number][] = [
    ...
    [0.0, 1.55, 0], [-0.7, 1.6, 0], [0.8, 1.45, 0],
    [-0.2, 2.25, 0], [0.6, 2.2, 0], [-0.9, 2.3, 0],
    [0.2, 2.95, 0], [-0.4, 2.9, 0], [0.9, 3.0, 0],
    [-0.6, 3.55, 0], [0.3, 3.6, 0],
    [0.0, 4.25, 0], [-0.5, 4.2, 0],
  ];
...
function addWardSeal(group: THREE.Group, y: number, frontZ: number): void {
  const wardMat = surfaceMat({
    color: 0xff4422, emissive: 0xdd2200, emissiveIntensity: 1.6,
    roughness: 0.3, metalness: 0.0, flatShading: !GFX.standardMaterials,
  });
...
function buildProceduralRewardChest(entityId: number): { group: THREE.Group; height: number } {
  ...
  const lid = new THREE.Mesh(
    new THREE.BoxGeometry(1.82, 0.42, 1.12),
    woodMat,
  );
  lid.position.y = 1.11;
  lid.castShadow = true;
  ...
  for (const bx of [-0.75, 0, 0.75]) {
    const band = new THREE.Mesh(
      new THREE.BoxGeometry(0.12, 1.38, 1.14),
      ironBandMat,
    );
  ...
  for (const sx of [-0.82, 0.82]) {
    for (const sz of [-0.54, 0.54]) {
      const stud = new THREE.Mesh(
        new THREE.CylinderGeometry(0.07, 0.07, 1.35, 6),
        ironBandMat,
      );
  ...
  const latch = new THREE.Mesh(
    new THREE.BoxGeometry(0.22, 0.18, 0.1),
    goldMat,
  );
...
function buildProceduralLockedChest(entityId: number): { group: THREE.Group; height: number } {
  ...
  const ring = new THREE.Mesh(
    new THREE.TorusGeometry(0.32, 0.06, 6, 18),
    wardMat,
  );
...
function buildSurfaceExit(): { group: THREE.Group; height: number } {
  ...
  for (const sx of [-1.8, 1.8]) {
    const wall = new THREE.Mesh(
      new THREE.BoxGeometry(0.5, 4.5, 4.5),
      walledMat,
    );
  ...
  const archTop = new THREE.Mesh(
    new THREE.BoxGeometry(4.2, 0.5, 0.5),
    archMat,
  );
  ...
  for (let s = 0; s < 5; s++) {
    ...
    const step = new THREE.Mesh(
      new THREE.BoxGeometry(stepW, stepH * (s + 1), stepD),
      stairMat,
    );
...
function buildDestructibleWall(entityId: number): { group: THREE.Group; height: number } {
  ...
  for (let row = 1; row < 6; row++) {
    const mortar = new THREE.Mesh(
      new THREE.BoxGeometry(7.2, 0.06, 0.86),
      crackMat,
    );
  ...
  for (let row = 0; row < 6; row++) {
    ...
    for (let col = 0; col < 5; col++) {
      const mortar = new THREE.Mesh(
        new THREE.BoxGeometry(0.06, blockTall, 0.86),
        crackMat,
      );
  ...
  for (const [cx, , cz] of chunkDefs) {
    ...
    const chunk = new THREE.Mesh(
      new THREE.BoxGeometry(cw, ch, 0.4),
      chunkMat,
    );
